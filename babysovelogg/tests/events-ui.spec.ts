import { test, expect, createBaby, setWakeUpTime, forceMorning, generateId } from "./fixtures";
...
test("GET /api/events with type filter narrows results", async ({ page }) => {
  ...
  await postEvent(page, [
    makeEvent("sleep.started", { babyId, startTime: new Date().toISOString(), sleepDomainId: generateId() }),
  ]);
  await postEvent(page, [
    makeEvent("diaper.logged", { babyId, time: new Date().toISOString(), type: "wet", diaperDomainId: generateId() }),
  ]);
