...
describe("baseline comparison", () => {
  it("all strategies summary", () => {
    const lines = [engine, ageDefault, yesterday, movingAvg].map(summaryLine);
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      "engine: 82 days, count 78% (64/82), nap MAE 58.3 min, bed MAE 39.7 min, nap bias +3.4, count bias +0.07
  ...
  it("per-month breakdown — engine vs baselines", () => {
    const lines = [engine, ageDefault, yesterday, movingAvg].map(perMonth);
    expect(lines.join("\n\n")).toMatchInlineSnapshot(`
      "engine 6mo: 5 days, count 20% (1/5), nap MAE 48 min, bed MAE 61.5 min, nap bias +24.2, count bias -0.8
      engine 7mo: 31 days, count 81% (25/31), nap MAE 49.4 min, bed MAE 41.9 min, nap bias -15.3, count bias +0.13
      engine 8mo: 28 days, count 89% (25/28), nap MAE 32.7 min, bed MAE 26.5 min, nap bias +8.7, count bias +0.11
      engine 9mo: 18 days, count 72% (13/18), nap MAE 162.7 min, bed MAE 49.6 min, nap bias +34.7, count bias +0.17
...
describe("engine beats baselines", () => {
  ...
  // NOTE: 3d-avg nap count (79%) slightly edges the engine (78%) because
  // the moving average adapts to transitions faster than the >60% mode threshold.
  // This motivates the dual-hypothesis transition logic.
  it("3d-avg nap count slightly beats engine (motivates transition logic)", () => {
    expect(movingAvg.result.napCountAccuracy).toBeGreaterThanOrEqual(engine.result.napCountAccuracy);
  });
});
