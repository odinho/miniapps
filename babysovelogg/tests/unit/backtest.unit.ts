...
describe("baseline", () => {
  ...
  it("per-month breakdown", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      "6mo: 5 days, count 60% (3/5), nap MAE 47.8, dur MAE 22.1, bed MAE 61.5, wake MAE 22, nap bias +30.5, count bias -0.4
      7mo: 31 days, count 74% (23/31), nap MAE 51.5, dur MAE 27.8, bed MAE 43.8, wake MAE 29.2, nap bias -7.6, count bias +0.13
      8mo: 28 days, count 89% (25/28), nap MAE 34.1, dur MAE 17.7, bed MAE 26.5, wake MAE 19, nap bias +11.5, count bias +0.11
      9mo: 18 days, count 83% (15/18), nap MAE 148.6, dur MAE 29.8, bed MAE 44.3, wake MAE 113, nap bias +38, count bias +0.06"
    `);
  });
  ...
  it("per-month with manual nap count", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      "6mo manual=3: 5 days, count 80% (4/5), nap MAE 54, dur MAE 23.9, bed MAE 68.1, wake MAE 21, nap bias +37.8, count bias +0.2
      7mo manual=2: 31 days, count 84% (26/31), nap MAE 51.8, dur MAE 28.3, bed MAE 41.9, wake MAE 29.2, nap bias -9.2, count bias +0.1
      8mo manual=2: 28 days, count 89% (25/28), nap MAE 34.1, dur MAE 17.7, bed MAE 26.5, wake MAE 19, nap bias +11.5, count bias +0.11
      9mo manual=1: 18 days, count 89% (16/18), nap MAE 160, dur MAE 30.1, bed MAE 41.4, wake MAE 113, nap bias +38.4, count bias -0.11"
    `);
  });

  it("combined summary", () => {
    expect(renderSummary(auto, "all")).toMatchInlineSnapshot(`"all: 82 days, count 80% (66/82), nap MAE 58.6, dur MAE 23.8, bed MAE 39.3, wake MAE 43.6, nap bias +9, count bias +0.07"`);
  });
  ...
  it("warm-up curve", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      "day 1-3: 3 days, count 33% (1/3), nap MAE 66.6, dur MAE 25.3, bed MAE 54.9, wake MAE 20, nap bias +39.7, count bias -0.67
      day 4-7: 4 days, count 50% (2/4), nap MAE 35.7, dur MAE 29.3, bed MAE 80.3, wake MAE 35.8, nap bias +9.6, count bias +0.5
      day 8-14: 7 days, count 86% (6/7), nap MAE 71.1, dur MAE 24.5, bed MAE 43, wake MAE 26.9, nap bias -51.2, count bias +0.14
      day 15+: 68 days, count 84% (57/68), nap MAE 58.9, dur MAE 23.1, bed MAE 35.6, wake MAE 47.1, nap bias +13.8, count bias +0.07"
    `);
  });
