...
describe("baseline", () => {
  ...
  it("per-month breakdown", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      ...
      9mo: 18 days, count 72% (13/18), nap MAE 162.7 min, bed MAE 53.7 min, nap bias +34.7, count bias +0.17"
    `);
  });
  ...
  it("per-month with manual nap count", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      ...
      9mo manual=1: 18 days, count 89% (16/18), nap MAE 171.1 min, bed MAE 55.7 min, nap bias +36, count bias -0.11"
    `);
  });

  it("combined summary", () => {
    expect(renderSummary(auto, "all")).toMatchInlineSnapshot(`"all: 82 days, count 78% (64/82), nap MAE 58.3 min, bed MAE 49.9 min, nap bias +3.4, count bias +0.07"`);
  });
  ...
  it("warm-up curve", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      ...
      day 15+: 68 days, count 82% (56/68), nap MAE 58.4 min, bed MAE 49.7 min, nap bias +9.7, count bias +0.12"
    `);
  });
