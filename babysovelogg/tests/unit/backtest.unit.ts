...
describe("baseline", () => {
  ...
  it("per-month breakdown", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      ...
      7mo: 31 days, count 74% (23/31), nap MAE 50.1, dur MAE 27, bed MAE 24.3, wake MAE 28.9, nap bias -7.2, count bias +0.13
      8mo: 28 days, count 89% (25/28), nap MAE 26.4, dur MAE 18.8, bed MAE 18.1, wake MAE 21.8, nap bias +3.3, count bias +0.11
  ...
  it("per-month with manual nap count", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      ...
      7mo manual=2: 31 days, count 84% (26/31), nap MAE 48.8, dur MAE 27.3, bed MAE 22, wake MAE 29, nap bias -5.7, count bias +0.1
      8mo manual=2: 28 days, count 89% (25/28), nap MAE 26.4, dur MAE 18.8, bed MAE 18.1, wake MAE 21.8, nap bias +3.3, count bias +0.11
      9mo manual=1: 22 days, count 91% (20/22), nap MAE 63.8, dur MAE 23.4, bed MAE 20, wake MAE 28.5, nap bias -13, count bias -0.09"
    `);
  });

  it("combined summary", () => {
    expect(renderSummary(auto, "all")).toMatchInlineSnapshot(`"all: 86 days, count 81% (70/86), nap MAE 44.5, dur MAE 23.1, bed MAE 22.6, wake MAE 25.6, nap bias -3, count bias +0.07"`);
  });
  ...
  it("warm-up curve", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      ...
      day 15+: 72 days, count 85% (61/72), nap MAE 40.2, dur MAE 22.4, bed MAE 21.9, wake MAE 26.1, nap bias -0.2, count bias +0.07"
    `);
  });
