...
describe("multi-baby backtest", () => {
  ...
  it("all babies summary", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      ...
      baby_2: 54 days, count 46% (25/54), nap MAE 62.9, dur MAE 28.9, bed MAE 59.8, wake MAE 74.8, nap bias +16.9, count bias -0.09
      baby_3: 50 days, count 52% (26/50), nap MAE 134.8, dur MAE 38.1, bed MAE 471.6, wake MAE 166.6, nap bias +18.2, count bias +0.32
      baby_5: 8 days, count 50% (4/8), nap MAE 119.9, dur MAE 29.6, bed MAE 1574, wake MAE 722, nap bias -17.1, count bias +0.5"
    `);
  });
  ...
  it("baby_1 per-month (longest dataset)", () => {
    ...
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      ...
      18mo: 27 days, count 100% (27/27), nap MAE 33.5, dur MAE 26.6, bed MAE 44.4, wake MAE 27.6, nap bias +10.9, count bias 0
      19mo: 27 days, count 100% (27/27), nap MAE 47.7, dur MAE 25.2, bed MAE 35.5, wake MAE 50.9, nap bias +30.4, count bias 0
      20mo: 29 days, count 100% (29/29), nap MAE 25.1, dur MAE 22, bed MAE 30.3, wake MAE 30.6, nap bias +9.7, count bias 0
      ...
      23mo: 26 days, count 100% (26/26), nap MAE 22.5, dur MAE 21.8, bed MAE 23.3, wake MAE 38.6, nap bias +5.9, count bias 0
      ...
      26mo: 7 days, count 100% (7/7), nap MAE 30.2, dur MAE 17, bed MAE 35.2, wake MAE 52.2, nap bias -2.2, count bias 0
      27mo: 7 days, count 100% (7/7), nap MAE 47.4, dur MAE 23.3, bed MAE 16.4, wake MAE 28.7, nap bias -12.5, count bias 0
  ...
  it("baby_2 per-month", () => {
    ...
    const lines = buckets.map((b) => renderSummary(b.result, b.label));
    expect(lines.join("\n")).toMatchInlineSnapshot(`
      "0mo: 4 days, count 100% (4/4), nap MAE 0, dur MAE 0, bed MAE 0, wake MAE 0, nap bias 0, count bias 0
      1mo: 3 days, count 100% (3/3), nap MAE 0, dur MAE 0, bed MAE 0, wake MAE 0, nap bias 0, count bias 0
