...
function seedDb() {
  ...
  // Run the same schema as server/db.ts initSchema
  db.exec(`
...
function cli(
  ...
  try {
    ...
    if (!opts?.expectFail) {
      ...
      throw new Error(`CLI failed unexpectedly (exit ${e.status}):\nstdout: ${stdout}\nstderr: ${stderr}`, { cause: err });
    }
    return {
...
test("nap → up lifecycle", () => {
  ...
  // Status should show sleeping
  const status = cli(["--json"]);
...
test("nap while already sleeping fails", () => {
  cli(["nap"]);
  const { exitCode, stderr } = cli(["nap"], { expectFail: true });
...
test("pause and resume during nap", () => {
  ...
  const pause = cli(["pause", "--json"]);
  expect(JSON.parse(pause.stdout).ok).toBe(true);

  const resume = cli(["resume", "--json"]);
...
test("resume without paused sleep fails", () => {
  cli(["nap"]);
  const { exitCode, stderr } = cli(["resume"], { expectFail: true });
  expect(exitCode).not.toBe(0);
...
test("sleeps --json returns array", () => {
  // Start and end a nap so there's data
  cli(["nap"]);
  cli(["up"]);

  const { stdout } = cli(["sleeps", "--json"]);
  const data = JSON.parse(stdout);
...
test("nap --at with HH:MM format", () => {
  const { stdout } = cli(["nap", "--at", "14:30", "--json"]);
  const data = JSON.parse(stdout);
  expect(data.ok).toBe(true);
  // The time should contain 14:30 in some form
  expect(data.startTime).toBeDefined();
});

test("nap --at with relative time format", () => {
  // Start nap 10 minutes ago, then end it now
  const { stdout: napOut } = cli(["nap", "--at", "-10m", "--json"]);
  expect(JSON.parse(napOut).ok).toBe(true);
