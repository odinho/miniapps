...
- [~] P4-QA  Adversarial + multi-day simulation review; fix findings — PARTIAL: guardrails unit-covered (in-window, blocked, max-nudge, gain threshold, never-wake, tie-break) + learner-cleanliness sim + API gate. REMAINING: a faithful multi-day getFamilyState simulation (assemble→suggest→accept→append tagged sleeps→advance) asserting overlap-minutes trend UP and neither baby's own sleep degrades; + an e2e once a live suggestion can be deterministically seeded.
