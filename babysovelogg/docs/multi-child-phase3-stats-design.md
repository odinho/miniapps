...
- [x] S2-3  Twin gantt child-lanes. New `buildTwinTimeline(children)` merges both twins' gantt blocks into ONE timeline: each date row carries both children's blocks in stacked per-child lanes (top=child0 --moon, below=child1 --peach-dark), x/w already shared (fixed gantt scale, same 30-day rows). `SleepTimelineChart` block gains optional `h` (defaults to full row height → single-baby/sibling unchanged). Twin branch renders one combined gantt + child legend; removed the per-child gantt from `twinChildPanel` (heatmap + wake stay two-up). Siblings keep two instances. Unit-tested (lane y/colour/height + null when no data); golden byte-identical; build/typecheck/lint + stats e2e 11 + unit 1027 + integration 124 green.
- [x] S2-4 (P3-2)  Overlap visualisation = parent downtime. New pure `stats/shared-sleep.ts`: `computeSharedSleepByDay` (merge each child's sleep intervals → intersect → bucket both-asleep minutes per local day, splitting overlaps at tz-midnight) + `buildSharedSleepChart` (per-day area, reuses time-series frame) + `avgSharedSleepPerDay`. A "Felles søvn (foreldrekvile)" section shows for ANY 2 children (twin + sibling), once, above the per-child panels, with a snitt/dag stat. Unit-tested (overlap, midnight split, no-overlap, today-exclusion, avg); e2e asserts it shows for 2 children + absent for 1. Single-baby unchanged; golden byte-identical; lint/typecheck/build + unit 1032 + integration 124 + stats e2e 11 green.
- [x] S2-5 (P3-3)  Cross-child comparison stats. New pure `stats/child-comparison.ts` (`buildComparisonRows`) → per-metric rows (Total søvn/dag, Nattesøvn/dag, Lurar/dag, Lengste nattestrekk) with each child's value + the absolute divergence (2 children). A "Samanlikning" table section shows for any 2 children, once, above the per-child panels. Unit-tested (values + divergence + <2 children → empty); e2e asserts it shows for 2 + absent for 1. Single-baby unchanged; golden byte-identical; lint/typecheck/build + unit 1034 + integration 124 + stats e2e 11 green.
- [x] S2-QA (P3-QA)  Adversarial Codex review of all of Step 2 (per-child panels, twin overlay + shared scales, gantt child-lanes, shared-sleep/parent-downtime, cross-child comparison). Confirmed single-baby /stats byte-identical (golden) + behaviourally unchanged. Two MUST-FIX applied in-unit: (1) DST midnight-split — `shared-sleep.ts` now splits at the TRUE next local midnight via `setHourInTz` (a spring-forward day is 23h, not 1440 min), with an Oslo regression test (1320/60); (2) `loadFullHistory()` gained a `catch { error = true }` so a failed full-history fetch surfaces instead of hanging. Three findings triaged → followups (gross-not-net spans across whole page = consistent w/ engine convention; full-history 1000-row API cap = pre-existing; zero-overlap shared-sleep hidden vs shown-as-0). lint/typecheck/golden/build + unit + integration + stats e2e all green.
