...
- [ ] S1-6b  Migrate Leggetid (bedtime: avg line+label via `underlay`) + Vakevindu (wakeScatter: band-rect + circle dots via `underlay`/`overlay`, no xLabels) to `TimeSeriesChart`. Visually identical; Codex-verify.
