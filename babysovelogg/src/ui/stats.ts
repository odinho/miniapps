...
function renderBarChart(weekStats: WeekStats): SVGElement {
  ...
  const maxMin = Math.max(
    60,
    ...days.map((d) => d.stats.totalNapMinutes + d.stats.totalNightMinutes),
  );
  ...
  days.forEach((day, i) => {
    ...
    if (nightH > 0) {
      svg.appendChild(
        svgEl("rect", {
          x: String(x),
          y: String(baseY - totalH),
          width: String(barW),
          height: String(nightH),
          rx: "4",
          fill: "var(--moon)",
        }),
      );
...
function section(title: string, children: (HTMLElement | SVGElement)[]): HTMLElement {
  ...
  for (const c of children) div.appendChild(c as Node);
  return div;
}
...
export async function renderStats(container: HTMLElement): Promise<void> {
  ...
  chartContainer.appendChild(renderBarChart(weekStats) as Node);
  // Legend
  ...
  view.appendChild(
    section("Søvntrendar", [
      el("div", { className: "stats-trends-table" }, [
        ...
        trendRow(
          "Total søvn/dag",
          formatDuration(avgTotalSleep7 * 60000),
          formatDuration(avgTotalSleep30 * 60000),
        ),
        trendRow(
          "Snitt lurvarighet",
          formatDuration(
        ...
        trendRow(
          "Nattesøvn",
          formatDuration(weekStats.avgNightMinutesPerDay * 60000),
          formatDuration(allStats.avgNightMinutesPerDay * 60000),
        ),
      ]),
    ]),
  );
