...
export function renderSettings(container: HTMLElement, opts?: { onboarding?: boolean }): void {
  ...
  if (isOnboarding) {
    ...
    form.appendChild(
      el("p", { style: { color: "var(--text-light)", marginBottom: "24px" } }, [
  ...
  form.appendChild(
    el("div", { className: "form-group" }, [el("label", null, ["Namn"]), nameInput]),
  );
  form.appendChild(
    el("div", { className: "form-group" }, [
      el("label", null, ["Termindato"]),
      dateInput,
      el("div", { style: { fontSize: "0.75rem", color: "var(--text-light)", marginTop: "4px" } }, [
  ...
  if (baby) {
    ...
    const napPills = napOptions.map((o) => {
      const pill = el(
        "button",
        { className: `type-pill ${selectedNapCount === o.value ? "active" : ""}` },
        [o.label],
      );
    ...
    form.appendChild(
      el("div", { className: "form-group" }, [
        ...
        el(
          "div",
          { style: { fontSize: "0.75rem", color: "var(--text-light)", marginBottom: "8px" } },
  ...
  if (baby) {
    ...
    const pottyPills = pottyOptions.map((o) => {
      const pill = el(
        "button",
        { className: `type-pill ${pottyEnabled === o.value ? "active" : ""}` },
        [o.label],
      );
      pill.addEventListener("click", () => {
        ...
        pottyPills.forEach(
          (p, i) =>
            (p.className = `type-pill ${pottyEnabled === pottyOptions[i].value ? "active" : ""}`),
        );
      });
      return pill;
    });
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    const payload: Record<string, unknown> = { name, birthdate };
    if (baby) {
...
function renderSleepInfoPanel(
  ageMonths: number,
  wakeWindow: { minMinutes: number; maxMinutes: number },
): HTMLElement {
  ...
  const rows = [
    {
      label: "Vakevindu",
      value: `${fmtMin(wakeWindow.minMinutes)} – ${fmtMin(wakeWindow.maxMinutes)}`,
    },
