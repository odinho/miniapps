...
3. **Production prompt fix shipped** (`src/ranking/prompt.ts`, lines 51 and 89). Final 30-batch validation on real DB data: **41.0% → 48.1% keep rate (+17.4%)**. Drops vs adds: 6 dropped vs 18 added on the first 10-batch sample — 3:1 add/drop ratio, net positive direction. **Re-rank at leisure to re-process your 72k photo backlog with the new default.** The code change only affects future ranking runs; existing state.db unchanged.
...
5. **Face-coverage experiment** (bonus, negative result). Tried feeding Immich's ML-identified face names to qwen_terse with a "maximize people coverage" prompt. User-match **dropped** from 93% (v2) to 83% — the prompt pushed the model to trade sharpness for coverage diversity. Face data is a useful tie-breaker, not a primary driver. See `IDEAS.md → Face-coverage signal` for revised approach.
6. **v2 prompt on gemma4:31b** (bonus, strong positive). Same v2 prompt on Stage B4's 15 groups: **15/15 user-match (100%)**, up from 12/15 (80%) with v1. Keep count 1.07 → 2.13. 7 of 13 added 2nd picks landed on another user-kept photo (54% hit rate — higher than qwen's 41% or gemini's 39%). Gemma4:31b is still slow (495s/group) so it's not the production choice, but **the v2 prompt generalises across models** — every model tested with v2 lifts meaningfully.
