...
if (!apiKey) {
  console.error("No OpenRouter API key");
  process.exit(1);
}
...
const batch = targetBatchId ? batches.find((b) => b.id === targetBatchId) : batches[0];
...
console.log(`\nBatch: ${batch.id}`);
console.log(
  `${batch.assets.length} photos, ${batch.source}, span: ${((batch.dateRange.end.getTime() - batch.dateRange.start.getTime()) / 60000).toFixed(0)}min`,
);
...
console.log(
  `Images: ${response.images?.length ?? 0}, Subgroups: ${response.similaritySubgroups?.length ?? 0}`,
);

if (response.images) {
  console.log(`\n--- Per-image assessments ---`);
  for (const img of response.images) {
    const stars = "★".repeat(img.suggestedStars) + "☆".repeat(Math.max(0, 5 - img.suggestedStars));
    ...
    console.log(`  ${stars} ${fn} — ${img.briefNote}${sg}`);
  }

  const starDist = [0, 0, 0, 0, 0, 0];
  for (const img of response.images) starDist[Math.min(img.suggestedStars, 5)]++;
  console.log(
    `\n  Star distribution: 0★:${starDist[0]} 1★:${starDist[1]} 2★:${starDist[2]} 3★:${starDist[3]} 4★:${starDist[4]} 5★:${starDist[5]}`,
  );
}
...
if (response.similaritySubgroups?.length) {
  ...
  for (const sg of response.similaritySubgroups) {
    ...
    console.log(
      `  Keep: ${sg.recommendedKeepIds.map((id) => batch.assets.find((a) => a.id === id)?.filename ?? id.slice(-20)).join(", ")}`,
    );
    console.log(
      `  Cull: ${sg.cullIds.map((id) => batch.assets.find((a) => a.id === id)?.filename ?? id.slice(-20)).join(", ")}`,
    );
