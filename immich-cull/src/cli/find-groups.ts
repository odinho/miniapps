...
const dbPath =
  process.argv[2] ?? resolve(import.meta.dirname ?? ".", "../../..", "facet/photo_scores_pro.db");
...
for (const g of groups) {
  if (g.assets.length === 12) {
    ...
    if (
      d.getFullYear() === 2024 &&
      d.getMonth() === 7 &&
      g.timeSpanMinutes > 1.3 &&
      g.timeSpanMinutes < 1.5
    ) {
...
for (const b of batches) {
  ...
  if (hasAug7) {
    ...
    b.assets
      .slice(0, 5)
      .forEach((a) => console.log("  ", a.filename, a.fileCreatedAt.toISOString().slice(0, 19)));
