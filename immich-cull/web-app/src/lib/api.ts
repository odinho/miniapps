...
export interface GroupSummary {
  id: string; index: number; count: number; timeSpanMinutes: number;
  avgDistance: number; decided: boolean; earliestDate: number; totalBytes: number;
  assets: AssetSummary[];
}

export interface AssetSummary {
  id: string; filename: string; date: string; rating: number | null;
  isFavorite: boolean; bytes: number;
}

export interface GroupDetail {
  id: string; count: number; timeSpanMinutes: number; avgDistance: number;
  totalBytes: number; decision: { keep: string[]; cull: string[]; skipped: boolean } | null;
  assets: AssetDetail[];
}
...
export interface LlmResult {
  batchSummary: string; overallConfidence: number;
  images: LlmImage[];
...
export interface LlmSubgroup {
  subgroupId: string; imageIds: string[]; subgroupType: string;
  recommendedKeepCount: number; recommendedKeepIds: string[];
  cullIds: string[]; rationale: string;
}

export interface Stats {
  totalGroups: number; decided: number; skipped: number;
  photosToKeep: number; photosToCull: number; remaining: number; cullBytes: number;
}
...
export async function decideGroup(id: string, keep: string[], cull: string[], skipped = false) {
  return (await fetch(`${BASE}/api/groups/${id}/decide`, {
